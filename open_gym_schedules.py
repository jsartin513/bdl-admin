...
Sheets produced:
  ...
  6 team          – bug fixes only (caption case + duplicate opponent)
...
# ---------------------------------------------------------------------------
# Styles (aligned with schedule_data/build_team_schedules.py patterns)
# ---------------------------------------------------------------------------
...
def parse_sections(ws) -> list[Section]:
    """Split worksheet into sections separated by blank rows + 'Home' header rows."""
...
def write_schedule_sheet(wb: Workbook, sheet_name: str, sections: list[Section]) -> None:
    ...
    for s_idx, section in enumerate(sections):
        ...
        c = ws.cell(row=current_row, column=1, value=f"Section {s_idx + 1}")
        c.font = FONT_SECTION
        c.fill = FILL_SECTION
        ...
        headers = ["Round", "Home", "Away", "Winner"]
        ...
        for g_idx, game in enumerate(section.games):
            ...
            row_vals = [game.round_label, game.home, game.away, None]
...
def build(input_path: Path, output_path: Path) -> None:
    ...
    out = Workbook()
    del_sheet = out.active
    out.remove(del_sheet)

    sections_2 = parse_sections(src["2 team"])
    write_schedule_sheet(out, "2 team", sections_2)

    sections_3 = parse_sections(src["3 team"])
    write_schedule_sheet(out, "3 team", sections_3)
    ...
    write_schedule_sheet(out, "4 team - Long", sections_4_fixed)
    sections_4_short = [section_take_first_n(s, 4) for s in sections_4_fixed]
    write_schedule_sheet(out, "4 team - Short", sections_4_short)

    sections_5 = parse_sections(src["5 team"])
    write_schedule_sheet(out, "5 team - Long", sections_5)
    sections_5_short = [section_take_first_n(s, 5) for s in sections_5]
    write_schedule_sheet(out, "5 team - Short", sections_5_short)

    sections_6 = fix_6team_sections(parse_sections(src["6 team"]))
    write_schedule_sheet(out, "6 team", sections_6)
